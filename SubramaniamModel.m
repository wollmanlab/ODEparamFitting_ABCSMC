...
function [T,X ,speciesArray] = SubramaniamModel( K,t,x0 )
    Prtot_e = K(1);
    Km_e = K(2);
    Prtot_x = K(3);
    Km_x = K(4);
    ...
    Gaitot = K(78);
    Gby_tot = K(79);
    % the species array is an array that keeps track of the algebraically calculated variables: IP3p, GiD, GRK_Gby, Cai2CaMGRK, Cai2CaM 
    speciesArray = zeros(20000,6);
    speciesInd = 0;
    % rowQuota is the number of rows that will need to be added on if the
    % speciesArray runs out of rows to be filled
    rowQuota = 1000;
    function [dx_dt] = F(t,x)
        ...
        if (speciesInd == size(speciesArray,1))
            % add more rows
            newRows = zeros(rowQuota,6);
            speciesArray = [speciesArray; newRows];
        end
        speciesInd = speciesInd +1;
        speciesArray(speciesInd,:) = [t,IP3p,GiD,GRK_Gby, Cai2CaMGRK, Cai2CaM];
        ...
        % [LR]GiD + T -> GaiT + Gby + D
        %v15
        v(15) = kf15*(GiD/(Km_gid_15+ GiD))*T*x(3);
        % [A]GaiT -> GaiD + Pi
        %v16
    ...
    [T,X] = ode15s(@F,[0,t],x0,options);
    % strip empty entries and repeated entries from speciesArray
    speciesArray = speciesArray(1:speciesInd,:);
    uniqueTime = unique(speciesArray(:,1));
    speciesArrayAdj = zeros(length(uniqueTime),6);
    speciesArrayAdj(:,1) = uniqueTime;
    for i = 1:size(speciesArrayAdj,1)
        [row,col] = find(speciesArray(:,1) == speciesArrayAdj(i,1));
        speciesArrayAdj(i,:) = speciesArray(max(row),:);
    end
    speciesArray = speciesArrayAdj;
end
